...
from models.crnn_fixed import FixedCRNN, CRNNLoss
from utils import create_data_loaders


class Trainer:
    def __init__(self, model, criterion, optimizer, scheduler, device, save_dir='./checkpoints_fixed'):
        self.model = model
    ...
    def validate(self, val_loader, dataset):
        ...
        with torch.no_grad():
            for images, targets, target_lengths, texts in tqdm(val_loader, desc='验证中'):
                images = images.to(self.device)
                targets = targets.to(self.device)
                target_lengths = target_lengths.to(self.device)
    ...
    def save_checkpoint(self, epoch, val_loss, val_acc, is_best=False):
        ...
        # 保存最新检查点
        latest_path = os.path.join(self.save_dir, 'latest_fixed.pth')
        torch.save(checkpoint, latest_path)
        
        # 保存最佳模型
        if is_best:
            best_path = os.path.join(self.save_dir, 'best_fixed.pth')
            torch.save(checkpoint, best_path)
            print(f"保存最佳模型: val_acc={val_acc:.4f}")
    
    def train(self, train_loader, val_loader, dataset, num_epochs):
        """完整训练循环"""
        best_val_acc = 0.0
        
        print(f"开始训练修复版CRNN，共{num_epochs}个epoch")
...
def main():
    parser = argparse.ArgumentParser(description='训练修复版CRNN验证码识别模型')
    parser.add_argument('--data_root', type=str, default='./data', help='数据根目录')
    parser.add_argument('--batch_size', type=int, default=32, help='批次大小')
    parser.add_argument('--num_epochs', type=int, default=50, help='训练轮数')
    parser.add_argument('--lr', type=float, default=1e-3, help='学习率')
    parser.add_argument('--save_dir', type=str, default='./checkpoints_fixed', help='模型保存目录')
